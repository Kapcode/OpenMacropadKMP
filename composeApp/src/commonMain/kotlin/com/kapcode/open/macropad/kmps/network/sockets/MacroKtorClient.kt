package com.kapcode.open.macropad.kmps.network.sockets
...
import com.kapcode.open.macropad.kmps.network.sockets.model.ControlCommand
import com.kapcode.open.macropad.kmps.network.sockets.model.controlMessage
import com.kapcode.open.macropad.kmps.network.sockets.model.heartbeatMessage
import com.kapcode.open.macropad.kmps.utils.Base64Utils
import com.kapcode.open.macropad.kmps.DeviceInfo
                    ...
                    for (frame in s.incoming) {
                        if (frame is Frame.Binary) {
                            try {
                                ...
                                dataModel.handle(
                                    onControl = { cmd: ControlCommand, params: Map<String, String> ->
                                        if (cmd == ControlCommand.AUTH_CHALLENGE) {
                                            ...
                                            if (challenge != null) {
                                                ...
                                                val response = controlMessage(
                                                    ControlCommand.AUTH_RESPONSE,
                                                    mapOf(
                                                        "signature" to Base64Utils.encode(sig),
                                                        "publicKey" to Base64Utils.encode(publicKey),
                                                        "metadata" to DeviceInfo.hardwareMetadata
                                                    )
                                                )
