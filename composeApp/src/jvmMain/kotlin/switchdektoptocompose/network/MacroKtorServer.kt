...
class MacroKtorServer(
    ...
    private suspend fun DefaultWebSocketServerSession.handleSession(client: ConnectedClient) {
        ...
        try {
            if (trustedDeviceManager.isBanned(client.id) || switchdektoptocompose.logic.PairingBanManager.isBanned(client.id)) {
                client.authStatus = AuthStatus.BANNED
                val remainingMs = switchdektoptocompose.logic.PairingBanManager.getRemainingBanTime(client.id)
