package com.kapcode.open.macropad.kmps.desktop.network
...
class MacroKtorServer(
    ...
    private suspend fun DefaultWebSocketServerSession.handleSession(client: ConnectedClient) {
        ...
        try {
            if (trustedDeviceManager.isBanned(client.id) || com.kapcode.open.macropad.kmps.desktop.logic.PairingBanManager.isBanned(client.id)) {
                client.authStatus = AuthStatus.BANNED
                val remainingMs = com.kapcode.open.macropad.kmps.desktop.logic.PairingBanManager.getRemainingBanTime(client.id)
                val reason = if (remainingMs > 0) {
                    "Device is temporarily banned for brute-force protection. Try again in ${remainingMs / 60000 + 1} minutes."
                } else {
                    "Device is banned"
                }
                send(Frame.Binary(true, controlMessage(ControlCommand.BANNED, mapOf("reason" to reason)).toBytes()))
    ...
    private suspend fun handleAuthResponse(client: ConnectedClient, params: Map<String, String>) {
        ...
        if (challenge != null && signature != null && publicKeyBase64 != null) {
            ...
            val isValid = IdentityManager.verifySignature(
