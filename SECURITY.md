...
*   **Mitigation:**
    ...
    *   **Pairing Brute-Force Protection:** ✅ **Fixed**. Implemented a "3-strikes" rule for the 6-digit verification code. If a device submits an incorrect code 3 times, it is automatically banned from the server to prevent exhaustive search attacks.
    ...
    *   **Trust Once (Session Isolation):** ✅ **Fixed**. Devices approved for a single session ("Trust Once") now have their trust explicitly revoked whenever the session ends or is replaced by a duplicate connection. This prevents "zombie" trust inheritance and ensures security policies are strictly enforced per-connection.
