...
### 1. Unauthorized Macro Execution (The "Joe vs. Sally" Problem)
- **Description**: The server currently accepts WebSocket connections from any client on the local network that knows its IP address. There is no explicit "Pairing Mode" or "Device Whitelist" implemented.
- **Abuse Scenario**: A malicious user ("Joe") on the same Wi-Fi can discover the server via UDP and trigger macros without the owner's ("Sally") consent. This could lead to remote command execution, system lockout, or data theft via automated keystrokes.
- **Mitigation Strategy**: Implement a device pairing workflow. The server should maintain a `paired_devices.json` whitelist. New connections from unknown Identity Keys should be blocked until approved via a desktop popup.

### 2. Immutability of Keys in RAM (String Leakage)
...
### 3. Lack of Hardware-Backed Security on JVM
...
### 5. Local Denial of Service (Keystore Corruption)
...
### 6. Keystore Password Brute Force
